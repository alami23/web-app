    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-display font-bold text-slate-900">Estimates & Quotations</h1>
            <p className="text-slate-500">Create and manage price quotes for custom furniture orders.</p>
          </div>
                  ...
                  <tr key={est.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-slate-900">{est.id}</span>
                        <span className="text-[10px] text-slate-400">Valid: {est.validUntil}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-700">{est.customer}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{est.items}</td>
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">৳{est.amount.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <span className={cn(
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button title="Convert to Invoice" className="p-2 hover:bg-emerald-50 rounded-lg text-emerald-600 transition-colors">
                          <ArrowRight size={18} />
                        </button>
                        <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 transition-colors">
                          <Printer size={18} />
                        </button>
                        <button className="p-2 hover:bg-rose-50 rounded-lg text-rose-400 transition-colors">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
