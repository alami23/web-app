    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-display font-bold text-slate-900">Invoice Returns</h1>
            <p className="text-slate-500">Handle product returns and refund adjustments.</p>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {returns.map((ret) => (
                  <tr key={ret.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">{ret.id}</td>
                    <td className="px-6 py-4 text-sm text-amber-600 font-medium">{ret.invRef}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">{ret.customer}</td>
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">৳{ret.amount.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <span className={cn(
                        "px-2.5 py-1 rounded-full text-[10px] font-bold uppercase",
                        ret.status === 'Approved' ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                      )}>
                        {ret.status}
                      </span>
                    </td>
