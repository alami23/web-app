    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-display font-bold text-slate-900 dark:text-slate-100">Business Reports</h1>
            <p className="text-slate-500 dark:text-slate-400">Analyze your furniture business performance with detailed insights.</p>
          </div>
          <div className="flex gap-2">
            <button 
              onClick={() => window.print()}
              className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            >
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-4 items-center">
          <div className="flex items-center gap-4 flex-1">
            <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
              ...
              <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Mar 01, 2024 - Mar 31, 2024</span>
            </div>
            <div className="h-6 w-px bg-slate-200 dark:bg-slate-800"></div>
            <div className="flex items-center gap-2">
              <Filter size={16} className="text-slate-400 dark:text-slate-500" />
              <select className="bg-transparent border-none outline-none text-sm font-medium text-slate-600 dark:text-slate-400">
          ...
          <button className="w-full md:w-auto px-6 py-2 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-xl text-sm font-bold">Apply Filters</button>
        </div>
        ...
        {reportType === 'Customer Due' ? (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Total Outstanding</p>
                <h3 className="text-2xl font-bold text-rose-600 dark:text-rose-400">৳{customerDueData.reduce((acc, curr) => acc + curr.totalDue, 0).toLocaleString()}</h3>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Total Due Customers</p>
                <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{customerDueData.length}</h3>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Avg. Due per Customer</p>
                <h3 className="text-2xl font-bold text-amber-600 dark:text-amber-400">৳{(customerDueData.reduce((acc, curr) => acc + curr.totalDue, 0) / customerDueData.length).toLocaleString()}</h3>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 className="font-bold text-slate-800 dark:text-slate-200">Customer Due List</h3>
                <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <span>Sorted by: <span className="font-bold text-slate-900 dark:text-slate-100 capitalize">{sortField.replace(/([A-Z])/g, ' $1')}</span></span>
                  <span className="uppercase font-bold text-slate-900 dark:text-slate-100">({sortOrder})</span>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {sortedDueData.map((customer) => (
                      <tr key={customer.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-rose-600 dark:text-rose-400">৳{customer.totalDue.toLocaleString()}</td>
                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{customer.lastInvoiceDate}</td>
                        <td className="px-6 py-4 text-right">
                          <button 
                            onClick={() => handleSendReminder(customer.name)}
                            className="p-2 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg transition-colors group relative"
                            title="Send Reminder"
                          >
                            <Send size={18} />
                            <span className="absolute bottom-full right-0 mb-2 hidden group-hover:block bg-slate-800 dark:bg-slate-700 text-white text-[10px] py-1 px-2 rounded whitespace-nowrap">
                              Send Reminder
                            </span>
                          </button>
                        </td>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100">৳{salesData.reduce((acc, curr) => acc + curr.sales, 0).toLocaleString()}</h3>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Total Profit</p>
                <h3 className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">৳{salesData.reduce((acc, curr) => acc + curr.profit, 0).toLocaleString()}</h3>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Avg. Daily Sales</p>
                <h3 className="text-2xl font-bold text-amber-600 dark:text-amber-400">৳{(salesData.reduce((acc, curr) => acc + curr.sales, 0) / salesData.length).toLocaleString()}</h3>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Conversion Rate</p>
                <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100">12.4%</h3>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 text-lg">{reportType} Performance Trend</h3>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-amber-500"></div>
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Sales</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="h-[350px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={salesData}>
                  ...
                  <Tooltip 
                    cursor={{fill: '#f8fafc'}}
                    contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)', backgroundColor: 'var(--tw-bg-opacity, #fff)' }}
                  />
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-6">Revenue Breakdown</h3>
            ...
            <div className="bg-slate-900 dark:bg-slate-800 p-6 rounded-3xl text-white shadow-xl shadow-slate-900/20 relative overflow-hidden">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 dark:text-slate-200">Detailed Sales Summary</h3>
            <button className="text-sm font-bold text-amber-600 dark:text-amber-400">View Full Ledger</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {[
                  ...
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-slate-100">{row.cat}</td>
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{row.items} units</td>
                    <td className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-slate-100">৳{row.rev.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm text-slate-500 dark:text-slate-500">৳{row.cost.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm font-bold text-emerald-600 dark:text-emerald-400">৳{row.profit.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <span className={cn(
                        "text-xs font-bold flex items-center gap-1",
                        row.growth.startsWith('+') ? "text-emerald-600 dark:text-emerald-400" : "text-rose-500 dark:text-rose-400"
                      )}>
